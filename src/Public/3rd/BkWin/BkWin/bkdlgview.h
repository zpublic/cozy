...
public:
	...
	void SetHeaderHeight(UINT uHeight, bool bRepostion = true)
	{
		CStringA strPos;

		strPos.Format("0,0,-0,%d", uHeight);
		m_bkHeader.SetAttribute("pos", strPos, TRUE);

		if (bRepostion)
		{
			_RepositionItems(TRUE);
		}
	}
protected:
...
public:
	void SetHeaderHeight(UINT uHeight, bool bRedraw = true)
	{
		m_richView.SetHeaderHeight(uHeight, bRedraw);
	}
