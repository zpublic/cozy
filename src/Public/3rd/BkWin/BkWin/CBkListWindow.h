...
protected:
	...
	BOOL OnMouseWhell(UINT nFlags, short zDelta, CPoint pt)
	{
        ...
        if (m_nwhellmove)
        {
            nScrollHeight = m_nwhellmove;
        }
        else
        {
            nScrollHeight = MOUSE_WHELL_MOVE * (abs(zDelta) / 120);
        }
		if (zDelta > 0)
		{
        ...
        if (m_pScrollCallback)
        {
			bHandedMouseWhell = m_pScrollCallback->NotifyScrollHook(GetCmdID(),abs(nScrollPos - nScrollHeight));
			if (!bHandedMouseWhell)
			{
				bHandedMouseWhell = m_pScrollCallback->NotifyMouseAction(GetCmdID(), WM_MOUSEWHEEL, (nScrollPos - nScrollHeight));
			}
        }
