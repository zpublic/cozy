...
public:
    ...
    void OnPaint(CDCHandle dc)
    {
        ...
        rcCheckBox.right = rcCheckBox.left + m_nCheckBoxSize;

#ifdef __SKIPTHEME__
		UINT uSubImg = _GetDrawSubImage();
		CBkSkinBase *pSkin = NULL;
		
		if (rcCheckBox.Height() > m_nCheckBoxSize)
		{
			rcCheckBox.top = rcCheckBox.top + (rcCheckBox.Height() - m_nCheckBoxSize)/2;
			rcCheckBox.bottom = rcCheckBox.top + m_nCheckBoxSize;
		}
        ...
        m_rcWindow.left += m_nCheckBoxSize + m_nCheckBoxSpacing;
    ...
    LRESULT OnNcCalcSize(BOOL bCalcValidRects, LPARAM lParam)
    {
        ...
        pSize->cx += m_nCheckBoxSize + m_nCheckBoxSpacing;
        pSize->cy = max(pSize->cy, m_nCheckBoxSize);
...
protected:
    ...
    BKWIN_DECLARE_ATTRIBUTES_BEGIN()
		...
		BKWIN_UINT_ATTRIBUTE("checkbox_size", m_nCheckBoxSize, FALSE)
...
public:
    ...
    void OnPaint(CDCHandle dc)
    {
        CRect rcCheckBox(0, 0, m_nRadioBoxSize, m_nRadioBoxSize);
        rcCheckBox.MoveToXY(m_rcWindow.left, m_rcWindow.top + (m_rcWindow.Height() - m_nRadioBoxSize) / 2);
        ...
        m_rcWindow.left += m_nRadioBoxSize + m_nRadioBoxSpacing;
    ...
    LRESULT OnNcCalcSize(BOOL bCalcValidRects, LPARAM lParam)
    {
        ...
        pSize->cx += m_nRadioBoxSize + m_nRadioBoxSpacing;
        pSize->cy = max(pSize->cy, m_nRadioBoxSize);
