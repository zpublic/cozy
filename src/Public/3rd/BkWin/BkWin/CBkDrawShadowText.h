...
public:
		...
		Gdiplus::ARGB crShaow = Gdiplus::Color::MakeARGB(125, 0xFF, 0xFF, 0xFF))
	{
...

		{
			...
			BkGdiplusFont	 myFont(dcMem, hFont);
		...
		Gdiplus::ARGB crShadow = Gdiplus::Color::MakeARGB(125, 0xFF, 0xFF, 0xFF))
	{
		...
		BkGdiplusFont	 myFont(hDC, hFont);
...

		{
			...
			if (bTextHighlight)
			{
				...
				if ((Gdiplus::Font *)(myFont))
				{
					DrawShadow(graphics, 6, layoutRect, szText, nCount, layoutRect, *((Gdiplus::Font *)myFont), colorText, colorShadow, format, bTextHighlight);
				}
			}
