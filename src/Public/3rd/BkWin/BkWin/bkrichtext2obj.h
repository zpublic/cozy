...
public:
	...
	virtual void MeasureString(PCWSTR szText, int nCount, IN OUT CRectF& rcDraw)
	{
		...
		else if ((dwTextMode & DT_VCENTER) == DT_VCENTER)
		{
			...
			rcDraw.top += nPosY - 0.5;
			rcDraw.bottom = rcDraw.top + sizeText.cy + 0.5;
		}
		else
		{
			rcDraw.bottom = rcDraw.top + sizeText.cy;
...
protected:
	...
	void InitImpl()
	{
		...
		m_pFontText = new BkGdiplusFont(m_dcHanle, hFont);
...
public:
	...
	virtual CRect GetRealContentRect() = 0;
};
...
public:
	...
	virtual CRect GetRealContentRect()
	{
		CRect r((int)m_rcObj.left, (int)m_rcObj.top, (int)m_rcObj.right, (int)m_rcObj.bottom);
		return r;
	}
protected:
...
public:
	...
	virtual CRect GetRealContentRect()
	{
		...
		while (iter != m_vecAtomObj.end())
		{
			IBkRichtextObj *pObj = *iter;
			if (pObj)
			{
				CRect rReal = pObj->GetRealContentRect();

				if (iter == m_vecAtomObj.begin())
				{
					rc = rReal;
				}
				else
				{
					if (rc.left > rReal.left) rc.left = rReal.left;
					if (rc.right < rReal.right) rc.right = rReal.right;
					if (rc.top > rReal.top) rc.top = rReal.top;
					if (rc.bottom < rReal.bottom) rc.bottom = rReal.bottom;
				}
			}
			++iter;
		}
