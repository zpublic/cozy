...
public:
	...
	HBKWND GetListItemBKHANDLE(UINT uItemID, int nItem)
	{
		CBkListWnd *pWndList = FindListWnd(uItemID);
		if (pWndList)
		{
			return pWndList->GetListItemBKHANDLE(nItem);
		}
    ...
    BOOL MoveListItemToIndex(UINT uItemID, HBKWND hBkListItem, int nIndex)
    {
        CBkListWnd *pWndList = FindListWnd(uItemID);
        if (pWndList == NULL) return FALSE;

        return pWndList->MoveListItemToIndex(hBkListItem, nIndex);
    }
	...
	BOOL SetListItemChildCtrlRichText(UINT uItemID, int nListItem, UINT uListItemChildID, LPCSTR lpszXml)
	{
		...
		if (!pWnd->IsClass(CBkRichText::GetClassName())
            && !pWnd->IsClass(CBkRichText2Wnd::GetClassName()))
        {
			return FALSE;
        }
    ...
    BOOL SetListItemNeedReDraw(UINT uItemID, int nListItem)
    {
        CBkListWnd *pWndList = FindListWnd(uItemID);
        if (pWndList == NULL) return FALSE;
        ...
        return pWndList->SetListItemNeedReDraw(nListItem);
    }

	BOOL SetListItemPaintHook(UINT uItemID, int nListItem, IBkWindowPaintHook* pPaintHook)
	{
		CBkListWnd *pWndList = FindListWnd(uItemID);
		if (pWndList == NULL) return FALSE;
		...
		return pWndList->SetListItemPaintHook(nListItem, pPaintHook);
	}
	...
	BOOL SetListItemChildCtrlPanelXml(UINT uItemID, int nListItem, UINT uListItemChildID, LPCSTR lpszXml)
	{
		...
		return pWndList->SetListItemChildCtrlPanelXml(nListItem, uListItemChildID, lpszXml);
	}

	BOOL SetListWndVScrollBarAttribute(UINT uItemID, LPCSTR lpszAttrib, LPCSTR lpszValue)
	{
		CBkListWnd *pWndList = FindListWnd(uItemID);
		if (pWndList == NULL) return FALSE;

		return pWndList->SetVScrollBarAttribute(lpszAttrib, lpszValue, FALSE);
	}
};
