    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (r == "master")
            {
                ...
                slave.Start(IPAddress.Loopback, 48360, new MasterPeerListener());
                var peer = new Peer() { EndPoint = new IPEndPoint(IPAddress.Loopback, 48361) };
                slave.Connect(peer);
                Console.ReadKey();
            ...
            else if (r == "master2")
            {
                MasterPeer slave = new MasterPeer();
                slave.Start(IPAddress.Loopback, 48360, new MasterPeerListener());
                var peer1 = new Peer() { EndPoint = new IPEndPoint(IPAddress.Loopback, 48361) };
                var peer2 = new Peer() { EndPoint = new IPEndPoint(IPAddress.Loopback, 48362) };
                var peer3 = new Peer() { EndPoint = new IPEndPoint(IPAddress.Loopback, 48363) };
                slave.Connect(peer1);
                slave.Connect(peer2);
                slave.Connect(peer3);
                Console.ReadKey();
            ...
            else if (r == "slave")
            {
                ...
                slave.Start(IPAddress.Loopback, 48361, new SlavePeerListener());
                Console.ReadKey();
                slave.Send("hehe");
                Console.ReadKey();
                slave.Stop();
            ...
            {
                SlavePeer slave = new SlavePeer();
                slave.Start(IPAddress.Loopback, 48362, new SlavePeerListener());
                SlavePeer slave2 = new SlavePeer();
                slave2.Start(IPAddress.Loopback, 48363, new SlavePeerListener());
                Console.ReadKey();
                slave.Stop();
                slave2.Stop();
            }
